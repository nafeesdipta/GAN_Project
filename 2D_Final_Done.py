...
def define_discriminator(input_shape_d, n_inputs=2):
    global g1_loss, g2_loss
...
def define_generator(latent_dim, input_shape_g1, n_outputs=2):
    global score
    model = Sequential()
    # model.add(Flatten())
    # print(latent_dim)
...
def define_generator2(latent_dim, input_shape_g2, n_outputs=2):
    global score
    model = Sequential()
    # model.add(Flatten())
    # print(latent_dim)
...
def generate_real(generator, x):
    ...
    #values = randn(x) + 0.5
    conv = [[1,0],[0,3]]
    xx,yy = np.random.multivariate_normal(mean,conv, x).T
    xx = xx.reshape(-1,1)
    yy = yy.reshape(-1,1)
    values = np.concatenate([xx,yy], axis=1)
    #print('sbbbbbbbbssssssss', values.shape)
    y = -ones((x, 1))
...
def generate_fake_samples(generator2, latent_dim, n, dataset):
    ...
    x_input = generate_latent_points(2, n)
...
def train(g_model, g_model2, d_model, gan_model, batch_size, size, n_epochs=150, n_eval=2):
    ...
    for i in range(n_epochs):
        for j in range(batch_per_epoch):
            ...
            dyb = np.concatenate([y_real_n, y_fake_n], axis=1)
            dx = np.concatenate([n1, n2])
            #dx = dx.reshape(-1, 1)
            #print ('sssssssssssss', dx.shape)
            #dxx = dxx.reshape(-1,1)
            #dcn = dcn.reshape(-1,1)
            #n1 = n1.reshape(-1,1)
            #n2 = n2.reshape(-1,1)
            ...
            final_plot(n1, n2, score_each,x_real, x_fake, dxx, score_concate, x_real_n, x_fake_n)
...
def final_plot(n1, n2, score_each, xr, xf, dxx, score_concate, x_real_n, x_fake_n):
    # pyplot.scatter(r_nn, c1_hist, color='green')
    global count
    # pyplot.scatter(f_nn, c1_hist, color='blue')
    pyplot.scatter(xr[:,0], xr[:,1]+5, color='black', label='Ground Value of the user') 
    pyplot.scatter(xf[:,0], xf[:,1]+5, color='red', label = 'Ground value of the non user')
    pyplot.scatter(dxx[:,0], score_concate[:,0], color='orange', label='Dis: Max(D(G(user))-D(G(non user)))')
    pyplot.scatter(dxx[:,1], score_concate[:,1], color='orange')
    pyplot.scatter(n1[0:32, :0], score_each[0:32, :0], color='green', label='G1 minimize user')
    pyplot.scatter(n1[0:32, :1], score_each[0:32, :1], color='green')
    pyplot.scatter(n2[0:32, :0], score_each[32:64, :0], color='blue', label = 'G2 maximize not user')
    pyplot.scatter(n2[0:32, :1], score_each[32:64, :1], color='blue')
