...
def define_discriminator(input_shape_d, n_inputs=1):
    ...
    model = Sequential()
    # model.add(Flatten())
    model.add(Dense(15, activation='relu', kernel_initializer='he_uniform', kernel_constraint=const,input_dim=n_inputs))
...
def train(g_model, g_model2, d_model, gan_model, batch_size, size, n_epochs=100, n_eval=2):
    ...
    for i in range(n_epochs):
        for j in range(batch_per_epoch):
            ...
            y_real_n = y_real_n.reshape(-1, latent_dim)
            y_fake_n = y_fake_n.reshape(-1, latent_dim)
            dis_loss = d1_loss-d2_loss
            g1_loss = d1_loss
            g2_loss = d2_loss
            with tf.GradientTape() as tape:
                grads = tape.gradient(dis_loss, d_model.trainable_variables)
                opt.apply_gradients(zip(grads, d_model.trainable_variables))
            ...
            with tf.GradientTape() as tape:
                ...
                pred = g_model2(n2, training=True)
                loss = wasserstein_loss(y_real_n, pred)
            ...
            #with tf.GradientTape() as tape:
            #    grads = tape.gradient(d1_loss, g_model.trainable_variables)
            #    opt.apply_gradients(zip(grads, g_model.trainable_variables))
            ...
            _, acc_real = d_model.evaluate(n1,y_real_n)
            _, acc_fake = d_model.evaluate(n2, y_real_n)
            ...
            final_plot(n1, n2, score_each,x_real, x_fake, dxx, score_concate, x_real_n, x_fake_n, g1, g2)
...
def final_plot(n1, n2, score_each, xr, xf, dxx, score_concate, x_real_n, x_fake_n, g1,g2):
    # pyplot.scatter(r_nn, c1_hist, color='green')
    global count
    # pyplot.scatter(f_nn, c1_hist, color='blue')
    pyplot.scatter(xr, (xr * 0)+2, color='black') 
    pyplot.scatter(xf, (xf * 0)+1, color='red')
    pyplot.scatter(dxx, score_concate, color='orange')
    # pyplot.plot('Nearest Neigbour','discriminator score')
    pyplot.scatter(n1[0:32, :], score_each[0:32, :], color='green')
    pyplot.scatter(n2[0:32, :], score_each[32:64, :], color='blue')
    pyplot.scatter(x_real_n[0:32, :], g1[0:32,:]+4, color='black')
    pyplot.scatter(x_fake_n[0:32, :], g2[0:32, :]+3, color='red')
